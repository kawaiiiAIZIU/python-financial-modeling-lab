...
小林刚拿到一笔奖学金，想请你帮他比较几种安排。你已经学过C语言，不必从“什么是程序”重新开始；这一章只做一件事：把熟悉的变量、判断、循环和函数换成Python写法，并做出第一台财富计算器。""",
    "01_金融与量化.ipynb": """# 第1章 金融与量化

财富计算器已经能运行，但小林发现自己还没回答最重要的问题：10,000元奖学金究竟可以放到哪里？你将和他一起追踪资金流向，把口头承诺画成现金流，再用Python比较不同选择。""",
    "02_现金流复利与贴现.ipynb": """# 第2章 现金流、复利与贴现

小林的第一份比较报告遇到了麻烦：今天的10,000元、三年后的12,000元和毕业后的收入不能直接放在一起比较。于是他要为所有现金流装上一把共同的“时间尺”。""",
    "03_债券与利率风险.ipynb": """# 第3章 债券与利率风险

校园创新社团想借10,000元，承诺每年付一笔利息，到期归还本金。小林没有立刻答应：这份未来付款合同今天值多少钱？市场利率改变后，旧合同为什么也会变价？""",
    "04_股票基金与市场交易.ipynb": """# 第4章 股票、基金与市场交易

研究完“把钱借给企业”，小林又收到另一种邀请：成为企业很小的一部分所有者。可即使决定买股票，他仍要弄清指数、基金、买价、卖价和交易费用怎样改变真正成交的结果。""",
    "05_金融数据与时间边界.ipynb": """# 第5章 金融数据与时间边界

小林下载了一张看起来整整齐齐的价格表，准备计算收益。你却发现里面混着重复日期、缺失值、拆股和尚未公布的数据。现在你们要像数据侦探一样，先保护现场，再决定哪些记录可以进入研究。""",
    "06_收益率与财富路径.ipynb": """# 第6章 收益率与财富路径

清洗后的价格终于可以使用，但“价格涨了多少”仍不等于“小林赚了多少”。股息、复合、年化和追加资金会把同一列价格变成不同的财富故事。""",
    "07_概率分布与抽样.ipynb": """# 第7章 概率、分布与抽样

历史数据只记录了一条已经发生的路。小林想讨论未来，却不愿把猜测伪装成预测。于是你们先写出一个透明的三情景模型，再用反复抽样观察模型可能产生什么。""",
    "08_风险度量与压力测试.ipynb": """# 第8章 风险度量与压力测试

小林拿到两项平均收益相近的方案，却发现“风险”没有一个万能答案。你们将用波动、回撤、尾部、压力情景和资本存续五副镜头检查同一项投资。""",
}

OPENINGS = {
    "00_从c语言到python.ipynb": """### 小林的量化研究档案：先造一台财富计算器

![C语言经验迁移到Python量化实验流程](assets/course/00_c_to_python_pipeline.png)

小林先给出一个最小委托：输入本金、收益率和年份，程序要返回每年的财富，还要能检查明显不合理的输入。你会先用熟悉的C语言思路拆解，再逐步换成Python、NumPy和图形。

这张概念图只说明工作关系，不承载精确数值。**本章交付物是一台可手算核对的财富计算器**；下一章，小林会真的拿它比较自己的10,000元奖学金。""",
    "01_金融与量化.ipynb": """### 小林的量化研究档案：10,000元该去哪里

![小林从奖学金出发比较金融选择](assets/course/01_financial_choices.png)

小林不会在开头背一串资产定义。他会先追问钱从谁流向谁，再依次遇到现金、借款、债券、股票和基金；每遇到一种选择，就把权利、现金流和风险记进同一份报告。

概念图只帮助辨认关系，不代表真实收益。**本章交付物是一页资产比较报告**；下一章将解决报告中“不同时点金额不能直接比较”的问题。""",
    "02_现金流复利与贴现.ipynb": """### 小林的量化研究档案：给现金流装上时间尺

![增长、贴现与多期现金流的时间价值](assets/course/02_time_value.png)

小林从100元、5%和3年这个可以手算的例子出发，先在今天与未来之间来回换算，再处理多笔现金流、通胀和一项校园实践计划。

概念图只解释增长与贴现的方向，不提供精确数值。**本章交付物是一台现金流比较器**；下一章会用它给一份债券合同定价。""",
    "03_债券与利率风险.ipynb": """### 小林的量化研究档案：要不要接受这份借款合同

![债券现金流、贴现与利率风险关系](assets/course/03_bond_mechanism.png)

社团给出的合同写着面值、票息和到期日。小林要先列出每一笔未来付款，再判断今天的合理价格；随后他会让市场利率上下变化，看看合同价格承受多大冲击。

概念图只说明合同关系。**本章交付物是一份债券报价与压力测试**；下一章将把债权人与股东的权利放在一起比较。""",
    "04_股票基金与市场交易.ipynb": """### 小林的量化研究档案：完成第一笔虚拟交易

![企业、股票、指数基金与交易市场关系](assets/course/04_market_mechanism.png)

小林先辨认股东拥有什么，再把多只股票合成指数，最后走进一个简化订单簿。屏幕上的价格不是自动成交价：订单大小、买卖价差和费用都会留下痕迹。

概念图不代表真实市场报价。**本章交付物是一张可解释的虚拟成交记录**；下一章将审查这些行情和成交数据是否可信。""",
    "05_金融数据与时间边界.ipynb": """### 小林的量化研究档案：抢救一张有问题的价格表

![金融数据从来源到可研究数据的审计流程](assets/course/05_data_pipeline.png)

你和小林先保存原始表，再逐项寻找乱序、重复、缺失、复权和发布时间问题。每一次删除、填补或对齐都必须留下理由，不能让一行清洗代码悄悄改写历史。

概念图只说明审计流程。**本章交付物是原始表、隔离表、清洗表和审计记录**；下一章只使用通过检查的数据计算收益。""",
    "06_收益率与财富路径.ipynb": """### 小林的量化研究档案：10,000元到底变成了多少

![价格、收益率、复合与财富路径关系](assets/course/06_returns_wealth.png)

小林会从100→110→99这个小例子开始，分清价格收益、总回报、算术平均、几何增长和年化。加入股息或定投以后，他还要说明现金流发生在什么时候。

概念图不提供任何收益数字。**本章交付物是一张收益口径审计卡和财富路径**；下一章会把一条历史路径扩展为许多假设情景。""",
    "07_概率分布与抽样.ipynb": """### 小林的量化研究档案：把不确定性写成透明模型

![可能结果、概率模型、抽样与统计量关系](assets/course/07_probability_sampling.png)

小林先写下亏20%、赚5%和赚30%三张情景卡，并明确每个概率只是教学假设。之后你们反复抽样，观察样本均值、分位数和尾部为什么会随样本改变。

概念图只说明“模型—抽样—统计量”的关系。**本章交付物是一份随机实验报告**；下一章将从这些样本中提取不同风险视角。""",
    "08_风险度量与压力测试.ipynb": """### 小林的量化研究档案：给资产做一次风险体检
...
PROJECT_HEADING_RENAMES = {
    "03_债券与利率风险.ipynb": ("本章总结与小项目", "项目交付：小林的债券压力测试"),
    "04_股票基金与市场交易.ipynb": ("本章总结与小项目", "项目交付：小林的第一笔虚拟交易"),
    "05_金融数据与时间边界.ipynb": ("本章总结与小项目", "项目交付：把脏数据变成可审计数据"),
    "06_收益率与财富路径.ipynb": ("本章总结与小项目", "项目交付：说明10,000元究竟怎样变化"),
    "07_概率分布与抽样.ipynb": ("本章总结与小项目", "项目交付：小林的随机实验报告"),
    "08_风险度量与压力测试.ipynb": ("本章总结与小项目", "项目交付：小林的五镜头风险体检"),
}
...
def story_from_card(source: str, anchor: str | None) -> str:
    ...
    if prior and question:
        paragraphs.append(f"小林把前面的线索带了过来：{prior}。接着他追问：**{question}**")
    elif question:
        paragraphs.append(f"小林接着追问：**{question}**")
    elif prior:
        paragraphs.append(f"小林把前面的线索带了过来：{prior}。")
    ...
    if boundary:
        paragraphs.append(f"小林还在报告旁边注明：{boundary}。")
    return "\n\n".join(paragraphs)
...
def polish_existing_bridge(source: str) -> str:
    ...
    source = re.sub(
        r"运行代码前，你和小林先([^\n]+?)；随后([^\n]+?)。(?=\n|$)",
        r"运行代码前，你和小林先做一个判断：\1。核对时再使用这条线索：\2。",
        source,
    )
    source = source.replace("运行代码前，你和小林先做一个判断：", "动手前：")
    source = source.replace("核对时再使用这条线索：", "核对时：")
    source = source.replace("最后把这条边界写进研究日志：", "小林还在报告旁边注明：")
