...
你在第5章完成了价格、股息与日期的数据清洗。现在他必须回答：**10,000元究竟变成了多少，这个结果使用了哪一种收益口径，追加资金以后还能不能用同一种方法解释？**
...
第6章告诉你“过去发生了什么”，却没有告诉他未来会出现哪一种收益。你将用三张情景卡建立一个**明确标注假设**的随机模型，再观察有限样本为什么会偏离模型中心。
...
你已经能描述收益分布，但“风险多大”仍不是一个单一问题。本章用起伏、从峰值下跌、尾部阈值、极端情景和资本存续五副镜头检查同一组收益。
...
在进入缩写和公式前，先做一个可以手工检查的排序实验。假设你记录了20次“一日损失金额”；损失越大越糟。先从小到大排序，再圈出最差10%，也就是最大的两个损失。
